  ...
  Object.values(selectedImages).forEach((images) => {
    images.forEach((image) => {
      ...
      scheduleLoad(
        ...
        (key) => textureLoadHandler(c, key)
      );
    });
  });
}

export function textureLoadHandler(c, key) {
  assetLoadHandler(key, c.texturesToLoad, c.texturesSentToLoadingManager, () => textureLoadComplete(c));
}
