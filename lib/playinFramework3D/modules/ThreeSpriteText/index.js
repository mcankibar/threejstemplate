...
export default class extends three.Sprite {
  ...
  _genCanvas() {
    ...
    // Calculate total height including descenders
    const metrics = ctx.measureText("yqpgj,"); // Text with descenders to measure
    const descent = metrics.actualBoundingBoxDescent || (this.fontSize * 0.2);
    const innerHeight = (this.fontSize * lines.length) + descent;
